import pytest
from parsers import convert_value_for_db, parse_telegram_dpt, parse_telegram_payload, format_dpt_name
...
def test_parse_telegram_payload():
    ...
    value_numeric, value_json, raw_data, dpt_str, dpt_main, dpt_sub = parse_telegram_payload(MockTelegram())
...
def test_format_dpt_name():
    ...
    # We can at least test that it returns the formatted string even if xknx lookup fails
    assert "1.001" in format_dpt_name(1, 1)
    assert "5.001" in format_dpt_name(5, 1)
    assert "9.001" in format_dpt_name(9, 1)
    assert format_dpt_name(None, None) is None

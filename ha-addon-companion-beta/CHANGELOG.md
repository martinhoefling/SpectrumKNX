...
- **Beta channel for the Home Assistant add-on**: pre-releases are now published as separate
  "Spectrum KNX (Beta)" and "Spectrum KNX (HA Companion, Beta)" add-ons, so the stable add-ons
  no longer push pre-release builds. Install whichever channel you want — not both (#427).

### Fixed

- **MCP endpoint required a trailing slash**: connecting a client to `http://<host>:8765/mcp`
  failed with "Method Not Allowed" (and a browser got the web UI instead), because the bare path
  fell through to the single-page-app route. `/mcp` now redirects to `/mcp/`, so both forms work
  (#426).
- **Pre-releases were offered as updates to stable installs**: every 2.0.0 beta was published as
  an ordinary release, so the update check advertised betas to everyone. Releases are now flagged
  correctly, and the check only offers versions from the channel you are on — a stable install is
  never sent to a beta, and a beta install still gets newer betas and, later, the final 2.0.0. The
  update dialog labels a pre-release as **BETA** (#427).
- **"Update did nothing" with Docker**: the shipped `docker-compose.yml` now sets
  `pull_policy: always` on the app service, so a moved tag is actually re-pulled instead of the
  cached local image being reused (#427).

### Changed

- **Update notification inside Home Assistant** points at the add-on page instead of suggesting an
  image pull, detected from the Supervisor environment rather than only from the ingress URL (#427).
- **MCP `query_telegrams`** now documents that a time range is bounded with `start_time`/`end_time`
  and that results can be truncated: the tool description spells out `limit_reached` and paging via
  `next_offset`, so an agent no longer reasons over a silently shortened slice of a busy bus (#428).
- **Docs**: new "Release Channels (Stable vs Beta)" section in `DEPLOYMENT.md` covering the Docker
  tag scheme and the add-on channels, plus the correct MCP URL for add-on users (ingress serves the
  UI only) (#426, #427).

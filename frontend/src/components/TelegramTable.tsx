    ...
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      <thead style={{ position: 'sticky', top: 0, background: 'var(--bg-panel)', zIndex: 10, backdropFilter: 'blur(8px)' }}>
        <tr style={{ textAlign: 'left', color: 'var(--text-dim)', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          <th style={{ padding: '0.75rem 1rem', width: '130px' }}><button className="sort-header" onClick={() => onSort('timestamp')}>TIME <SortIcon column="timestamp" /></button></th>
          <th style={{ padding: '0.75rem 1rem', width: '200px' }}><button className="sort-header" onClick={() => onSort('source_address')}>SOURCE <SortIcon column="source_address" /></button></th>
          <th style={{ padding: '0.75rem 1rem', width: '240px' }}><button className="sort-header" onClick={() => onSort('target_address')}>TARGET <SortIcon column="target_address" /></button></th>
          {visibleColumns.type && <th style={{ padding: '0.75rem 1rem', width: '110px' }}><button className="sort-header" onClick={() => onSort('simplified_type')}>TYPE <SortIcon column="simplified_type" /></button></th>}
          {visibleColumns.dpt && <th style={{ padding: '0.75rem 1rem', width: '140px' }}><button className="sort-header" onClick={() => onSort('dpt_name')}>DPT <SortIcon column="dpt_name" /></button></th>}
          <th style={{ padding: '0.75rem 1rem', minWidth: '180px' }}><button className="sort-header" onClick={() => onSort('value_numeric')}>VALUE <SortIcon column="value_numeric" /></button></th>
        </tr>
      </thead>
      <tbody>

  ...
  return (
    <div className="glass" style={{ height: 'calc(100vh - 4rem)', display: 'flex', flexDirection: 'column', position: 'relative' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead style={{ position: 'sticky', top: 0, background: 'var(--bg-panel)', zIndex: 10, backdropFilter: 'blur(8px)' }}>
            <tr style={{ textAlign: 'left', color: 'var(--text-dim)', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              <th style={{ padding: '0.75rem 1rem', width: '130px' }}><button className="sort-header" onClick={() => handleSort('timestamp')}>Time <SortIcon column="timestamp" /></button></th>
              <th style={{ padding: '0.75rem 1rem', width: '200px' }}><button className="sort-header" onClick={() => handleSort('source_address')}>Source <SortIcon column="source_address" /></button></th>
              <th style={{ padding: '0.75rem 1rem', width: '240px' }}><button className="sort-header" onClick={() => handleSort('target_address')}>Target <SortIcon column="target_address" /></button></th>
              {visibleColumns.type && <th style={{ padding: '0.75rem 1rem', width: '110px' }}><button className="sort-header" onClick={() => handleSort('simplified_type')}>Type <SortIcon column="simplified_type" /></button></th>}
              {visibleColumns.dpt && <th style={{ padding: '0.75rem 1rem', width: '140px' }}><button className="sort-header" onClick={() => handleSort('dpt_name')}>DPT <SortIcon column="dpt_name" /></button></th>}
              {visibleColumns.data && <th style={{ padding: '0.75rem 1rem', width: '160px' }}>Data</th>}
              <th style={{ padding: '0.75rem 1rem', minWidth: '180px' }}><button className="sort-header" onClick={() => handleSort('value_numeric')}>Value <SortIcon column="value_numeric" /></button></th>
            </tr>
          </thead>
          <tbody>
            {sortedTelegrams.length === 0 ? (
              ...
              <tr key={`${t.timestamp}-${idx}`} className="log-row" style={{ borderBottom: '1px solid var(--border-color)', fontSize: '0.8125rem' }}>
                ...
                <td style={{ padding: '0.75rem 1rem' }}>
                  ...
                  {visibleColumns.delta && (t as any).deltaStr && <div style={{ fontSize: '0.65rem', color: 'var(--text-dim)', marginTop: '0.1rem' }}>{(t as any).deltaStr}</div>}
                </td>
      ...
      <style>{`
        ...
        .subtitle-name { font-size: 0.7rem; color: var(--text-dim); margin-top: 0.15rem; overflow: hidden; text-overflow: ellipsis; whiteSpace: nowrap; }
